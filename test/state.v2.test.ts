...
contract('BStable proxy', async accounts => {
    ...
    describe('获取用户状态数据', async () => {
        ...
        it('获取数据', async () => {
            ...
            let p1Volume = new BigNumber(p1VolumeStr);
            let p2Volume = new BigNumber(p2VolumeStr);
            let p1TotalSupplyStr = await p1.totalSupply();
            let p2TotalSupplyStr = await p2.totalSupply();
            let p1TotalSupply = new BigNumber(p1TotalSupplyStr);
            let p2TotalSupply = new BigNumber(p2TotalSupplyStr);
            ...
            let p1InterestRate = p1Volume.multipliedBy(p1Fee).multipliedBy(new BigNumber(1).minus(p1AdminFee));
            let p2InterestRate = p2Volume.multipliedBy(p2Fee).multipliedBy(new BigNumber(1).minus(p2AdminFee));
