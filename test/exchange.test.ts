    ...
    describe('测试兑换', async () => {
        ...
        it('随机兑换', async () => {
            ...
            for (; true;) {
                ...
                let inCoin = await stableCoinContract.at(poolInfo[1][randI]);
                let balanceI = await inCoin.balanceOf(account);
                let randPow = Math.floor(Math.random() * 4);
                // 乘以0.4减少入金过大的情况
                let amt = new BigNumber(balanceI).multipliedBy(randPercent).multipliedBy(new BigNumber(0.4).pow(randPow)).div(100).toFixed(0, BigNumber.ROUND_DOWN);
                // let amt = '100000000000000000000';
                await proxyInstance.exchange(poolIndex, randI, randJ, amt, 0, { from: account }).catch(e => {
                    console.log(e);
                });
